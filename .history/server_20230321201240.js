...
app.post('/continue-conversation', async (req, res) => {
  try {
    ...
    const response = await openai.createChatCompletion({
     ...
     if (responseData && responseData.length > 0 && responseData[0].choices) {
       const aiMessage = responseData[0].choices
       .slice(1) // Skipp the first result
        .map(choice => choice.delta && choice.delta.content ? choice.delta.content : '')
         .join('')
         .trim();
