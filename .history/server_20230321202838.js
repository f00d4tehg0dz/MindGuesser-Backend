  ...
  try {
    ...
    for (const chunk of responseData) {
      if (
        ...
        aiMessageList.push(chunk.choices[0].delta.content);
      }
    }

    const aiMessage = aiMessageList.join('').trim();
    saveMessage(conversationId, 'ai', aiMessage);
    res.status(200).json({ message: aiMessage, messageList: aiMessageList });
